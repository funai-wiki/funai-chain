...
| [`FunAI_V6_Byzantine_Test_Plan_KT.md`](FunAI_V6_Byzantine_Test_Plan_KT.md) | ❌ not started | Doc landed 2026-04-27 (this week). Zero scenarios implemented. | Implement 30-scenario Go fuzzer (testing.F or hand-rolled harness); wire `make test-byzantine-quick` (100 rounds per scenario, every PR) and `make test-byzantine-full` (10 000 rounds, nightly); resolve the `jail_count` reset-50-vs-decay-1000 contradiction (see [`wiki/jail-and-slashing.md`](../../wiki/jail-and-slashing.md)) before the fuzzer can produce a meaningful pass/fail signal. |
